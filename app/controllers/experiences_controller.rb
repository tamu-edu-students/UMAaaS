...
class ExperiencesController < ApplicationController
  ...
  def new
    ...
    newExperience = Experience.create(title: params[:experience][:title],
                                      experience: params[:experience][:experience], rating: params[:experience][:rating], tags: tagArrayFixed, user_id: current_user.id, program_id: params[:id])

    newExperience.image.attach(params[:image]) if params[:image]

    puts 'PARAMS TIME'
    puts params[:experience][:title]

    if params.key?(:yelp_id)
      # has a Yelp location selected
      YelpLocation.create(experience_id: newExperience.id, name: params[:yelp_name],
                          address: params[:yelp_address], alias: params[:yelp_alias], yelp_id: params[:yelp_id], url: params[:yelp_url], image_url: params[:yelp_image_url], rating: params[:yelp_rating], yelp_tags: params[:yelp_tags])
    end
  ...
  def create_comment
    ExperienceComment.create(comment: params[:commentText], rating: params[:rating],
                             user_id: current_user.id, experience_id: params[:experienceId])
  ...
  def search(term, location)
    ...
    response = HTTP.auth("Bearer #{Rails.configuration.YELP_API_KEY}").get(url, params:)
    response.parse
  end
  ...
  def delete
    ...
    if experience.nil?
      ...
      redirect_to root_path and return
    elsif (experience.user_id != current_user.id) && !current_user.admin
      flash[:alert] = 'You are not authorized to delete this post!'
      redirect_to root_path and return
    end
  ...
  def delete_comment
    ...
    if experienceComment.nil?
      flash[:alert] = 'Post not found, error deleting!'
      redirect_to root_path and return
    elsif (experienceComment.user_id != current_user.id) && !current_user.admin
      flash[:alert] = 'You are not authorized to delete this post!'
      redirect_to root_path and return
    end
    # done checking for unauthorized deletions
    ...
    rating_sum = 0 if rating_sum.nil?
    rating_sum += @experience.rating # add the original rating
    ...
    @experienceDivId = "portal-experience-wrapper-#{@experience.id}"
  ...
  def edit
    ...
    return if @experience.tags.nil?

    @experience.tags = @experience.tags[1..-2]
    # tags are stored with a leading & trailing comma, this strips those off
  end
  ...
  def update
    ...
    @experience = Experience.find params[:id]
    @experience.update_attributes(title: params[:experience][:title],
                                  experience: params[:experience][:experience], rating: params[:experience][:rating], tags: tagArrayFixed)
    ...
    if params.key?(:yelp_id)
      # has a Yelp location selected
      YelpLocation.create(experience_id: params[:id], name: params[:yelp_name],
                          address: params[:yelp_address], alias: params[:yelp_alias], yelp_id: params[:yelp_id], url: params[:yelp_url], image_url: params[:yelp_image_url], rating: params[:yelp_rating], yelp_tags: params[:yelp_tags])
    end
